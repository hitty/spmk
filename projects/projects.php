...
if( $this_page->first_instance ){
    ...
    $GLOBALS['css_set'][] = '/projects/css/jquery-ui.min.css';
    $GLOBALS['js_set'][] = '/js/jquery-ui.min.js';
    $GLOBALS['js_set'][] = '/projects/js/jquery.list.js';
    ...
    $GLOBALS['js_set'][] = '/projects/js/script.js';
    ...
    $GLOBALS['js_set'][] = '/projects/js/jquery.form.edit.js';
...
$this_page->menuAdd( 'Номинации', 'projects/nominees', 1, false, false, false );
$this_page->menuAdd( 'Жюри', 'projects/users', 1, false, false, false );
$this_page->menuAdd( 'Новости', 'projects/news', 1, false, false, false );
$this_page->menuAdd( 'Фото и видео', 'projects/media', 1, false, false, false );
$this_page->menuAdd( 'Партнеры', 'projects/partners', 1, false, false, false );
$this_page->menuAdd( 'Страницы', 'projects/static_pages', 1, false, false, false );
$this_page->menuAdd( 'Настройки', 'projects/settings', 1, false, false, false );
if( $auth->id_group == 101 ){
    $this_page->menuAdd( 'URLы', 'projects/pages', 1, false, false, false );
    $this_page->menuAdd( 'Сео', 'projects/seo', 1, false, false, false );
}
...
if( !empty( $admin_mapping[ $action ] ) || ( !empty( $this_page->page_parameters[1] ) && !empty( $admin_mapping[ $this_page->page_parameters[1] ] ) ) ) {
    ...
    if( 
        ...
        if(!empty($admin_module['module'])){
            ...
            $module_file = Host::$root_path.'/modules/'.$admin_module['module'].'/projects/client.php';
            if(file_exists($module_file)) {
                ...
                $tpl = new Template($module_template, '/modules/'.$admin_module['module'] . '/projects');
                $module_content = $tpl->Processing();                        
...
if(empty($module_content) && !empty( $module_template ) ) {
    $tpl = new Template($module_template, '/projects');
    $module_content = $tpl->Processing();                        
