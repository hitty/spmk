        ...
        init : function(options) {
            ...
            return this.each(function() {
                ...
                var settings = $data.settings = $.extend({
                    ...
                    'fileSizeLimit'   : 15120*15120,        // Maximum allowed size of files to upload
                    'fileType'        : 'image',              // Type of files allowed (image, etc)
                    'formData'        : {
                                        'RSASESSIONID'  : _sessId,
                    ...
                    onInit    : function() {
                        if(settings.first_instance === true){
                                            ...
                                            $data.showUploadedPhotos(settings.idobject);
                                        }
                            ...
                            xhr[file.name].addEventListener('load', function(e) {
                                if (this.readyState == 4) {
                                    ...
                                    if (this.status == 200) {
                                        ...
                                        if ( !jsonResponse.error && file.xhr.responseText !== 'Invalid file type.') {
                                            $data.uploadComplete(e, file, uploadAll);
                                        } else {
                                            $data.error( jsonResponse.error ? jsonResponse.error : file.xhr.responseText, file, uploadAll);
                                        }
                    ...
                    if ($.inArray('onError', settings.overrideEvents) < 0) {
                        ...
                        switch(errorType) {
                            ...
                            case 'FILE_SIZE_LIMIT_EXCEEDED':
                                ...
                                break;
                            default:
                                errorMsg = typeof errorType == 'string' ? errorType : 'Неизвестная ошибка';
                                break;
                        }
                        ...
                        $data.removeQueueItem(file, 0, 2000 )
                    }
                    // Trigger the error event
                    if (typeof settings.onError === 'function') {
                    ...
                    if ($.inArray('onUploadComplete', settings.overrideEvents) < 0) {
                        ...
                        if(xhr[file.name].responseText.length>0){
                            ...
                            if(obj1.ok){
                                ...
                                _this_item.attr( 'data-id_obj', obj1.list.file_id )
                                    .attr( 'data-img', obj1.list.fullname  )
                                    .attr('style', 'background-image:url(' + obj1.list.fullname + ')');
                                _this_item.children( '.filename_wrap, .fileinfo_wrap, .progressbar-container').remove();
                                $data.appendManagePart( _this_item,obj1.list.file_id );
                ...
                $data.showUploadedPhotos = function (_idobject) {
                    console.log(init_selector.attr('name'));
                    jQuery.ajax({
                            ...
                            if(typeof(msg)=='object') {
                                if(msg.ok) {
                                    for(i=0;i<msg.list.length;i++){
                                        ...
                                        var _photo_name = typeof msg.list[i].direct_name == "boolean" ? msg.list[i].name : '/'+msg.folder+'/sm/'+msg.list[i].subfolder+'/'+msg.list[i].name ;
                                        jQuery('#'+settings.id + '_queue').append('\
                                                <li id="preload_'+i+'" class="'+mainPhotoClass+'">\
                                                    <div class="itemsContainer" data-id_obj="'+msg.list[i].id+'" data-img="' + _photo_name +'" style="background-image:url(' + _photo_name + ')">\
                                                    </div>\
                                                </li>');
                ...
                $data.appendManagePart = function(_this_item, list) {                
                    ...
                    var buttonSetMain = settings.buttonSetMain == true ? '<div class="boxcaption-wrap"><div class="boxcaption_main" data-icon="star" title="Сделать фотографию основной"></div>' : '<div class="boxcaption-wrap">';
                    
                    _this_item.parents('li').append('\
                            ' + buttonSetMain + '\
                                <div class="boxcaption_rotate" data-icon="rotate_right" title="Повернуть фотографию"></div><div class="boxcaption_del"  data-icon="delete" title="Удалить фотографию"></div></div>\
                            \
                    ');
                    
                    //установка заголовка для фотографии
                        ...
                        success: function(msg){
                            if(typeof(msg)=='object'){
                                    ...
                                    if($data.queue.count<settings.queueSizeLimit) {
                                        jQuery('.uploadifyButton', '#'+settings.id+'_queue').removeClass('inactive').removeClass('inactive');
                                    }
                                    //поиск главной фотографии
                ...
                $data.rotateFile = function(file) {
                    var _parent = file.closest('li');
                    var _el = jQuery( '.itemsContainer', _parent );
                    var _input = _parent.closest('ul').prev('.mUploadImg').children('input[type=file]');
                    jQuery.ajax({
                        type: "POST", async: true,
                        ...
                        success: function(msg){
                            if(typeof(msg)=='object'){
                                if(msg.ok){
                                    _el.css( { 'background-image' : 'url(' + _el.data('img') + '?' + Math.random() + ')' } )
                                }
                            }
                        },
                        error:function (xhr, ajaxOptions, thrownError){
                ...
                if (window.File && window.FileList && window.Blob && (window.FileReader || window.FormData)) {
                    ...
                    $data.queueEl = $('<ul id="' + settings.queueID + '" class="photo_upload_queue" />');
                    $wrapper.after($data.queueEl);
                    
                    $data.uploadli =  $('<li />', {
                        'class' : 'uploadifyButton',
                        'title'    : settings.buttonText,
                        'data-icon'    : 'photo_upload'
                    });
