...
// добавление title
$this_page->manageMetadata( array( 'title' => 'Проекты' ) );
...
switch( $action ){
            ...
            if($ajax_mode){
                ...
                switch($action){
                    case 'list':
                        ...
                        if(!empty($id)){
                            $list = Photos::getList( 'projects', $id );
                            if(!empty($list)){
                                $ajax_result['ok'] = true;
                    ...
                    case 'setMain':
                        ...
                        if(!empty($id_photo)){
                            $res = Photos::setMain( 'projects', $id, $id_photo);
                            if(!empty($res)){
                                $ajax_result['ok'] = true;
                    ...
                    case 'rotate':
                        ...
                        $id_photo = Request::GetInteger( 'id_photo', METHOD_POST );                
                        if(!empty($id_photo)){
                            $res = Photos::rotatePhoto( 'projects', $id_photo );
                            if(!empty($res)){
                                $ajax_result['ok'] = true;
                    ...
                    case 'setTitle':
                        ...
                        if(!empty($id)){
                            $res = Photos::setTitle('projects',$id, $title);
    ...
    case 'edit':
        ...
        if($action=='add'){
            ...
            $info = $db->prepareNewRecord($sys_tables['projects']);
            //временный ID для связанных таблиц
            if( empty( Session::GetInteger( 'common_projects_add' ) ) ) {
                $common_projects_add = mt_rand( 800000000, 1000000000 );
                Session::SetInteger( 'common_projects_add', $common_projects_add );
            }
            Response::SetInteger( 'common_edit', Session::GetInteger( 'common_projects_add' ) );
            
        } else {
            // получение данных из БД
            $info = $db->fetch("SELECT *
                                FROM ".$sys_tables['projects']." 
                                WHERE id=?", $id) ;
            if( empty( $info ) ) Host::Redirect( '/manage/projects/add/' );
            Response::SetInteger( 'id_projects', $id );
        }
        // перенос дефолтных (считанных из базы) значений в мэппинг формы
        foreach( $info as $key=>$field ){
            if( !empty( $mapping['projects'][$key] ) ) $mapping['projects'][$key]['value'] = $info[$key];
        }
        ...
        if( !empty( $post_parameters['submit'] ) ){
            ...
            foreach( $post_parameters as $key => $field ){
                if( !empty( $mapping['projects'][$key] ) ) $mapping['projects'][$key]['value'] = $post_parameters[$key];
            }
            // проверка значений из формы
            $errors = Validate::validateParams( $post_parameters, $mapping['projects'] );
            // выписывание ошибок в мэппинг формы (для отображения ошибочных полей)
            foreach( $errors as $key => $value ){
                if( !empty( $mapping['projects'][$key] ) ) $mapping['projects'][$key]['error'] = $value;
            }
            ...
            if( empty( $errors ) ) {
                ...
                foreach($info as $key=>$field){
                    if(isset($mapping['projects'][$key]['value'])) $info[$key] = $mapping['projects'][$key]['value'];
                }
                ...
                // сохранение в БД
                if($action=='edit'){
                    $res = $db->updateFromArray($sys_tables['projects'], $info, 'id') or die($db->error);
                } else {
                    $res = $db->insertFromArray($sys_tables['projects'], $info, 'id');
                    if(!empty($res)){
                        $new_id = $db->insert_id;
                        
                        //обновление временных данных
                        //перенос фотографий
                        Photos::setMain( 'projects', $new_id, false, 'id_parent',  Session::GetInteger( 'common_projects_add' ) );
                        Session::SetInteger( 'common_projects_add', 0 );
                        // редирект на редактирование свеженькой страницы
                        if(!empty($res) && empty( $ajax_mode ) ) {
                            header('Location: '.Host::getWebPath('/manage/projects/edit/'.$new_id.'/'));
                            exit(0);
                        }
                    }
                }
            ...
            if( $action == 'add' ) {
                ...
                Photos::DeleteAll( 'projects', false, Session::GetInteger( 'common_projects_add' ) );
            }
        }
        ...
        Response::SetArray( 'data_mapping', $mapping['projects'] );

        if( ! ( !empty( $ajax_mode ) && !empty( $post_parameters['submit'] ) ) )  $module_template = '/modules/projects/manage/templates/edit.html';
    ...
    case 'delete':
        if( $ajax_mode ){
            ...
            if( empty( $post_parameters['submit'] ) ) {
                ...
                Response::SetString( 'title', 'Проекты' );
        ...
        break;
    default:
        $module_template = '/modules/projects/manage/templates/list.html';
        // формирование списка
        $conditions = array( );
        if(!empty($filters)){
            if(!empty($filters['title'])) $conditions['title'] = $sys_tables['projects'].".`title` LIKE '%".$db->real_escape_string($filters['title'])."%'";
            if(!empty($filters['published'])) $conditions['published'] = $sys_tables['projects'].".`published` = ".$db->real_escape_string($filters['published'])."";
        }
        // формирование списка для фильтра
        $condition = implode(" AND ",$conditions);        
        // создаем пагинатор для списка
        $paginator = new Paginator( $sys_tables['projects'], 30, $condition );
        ...
        // ссылка пагинатора
        $paginator->link_prefix = '/manage/projects'                // модуль
        ...
        // формирование списка
        $list = CommonDb::getList( 'projects', $paginator->getLimitString($page), $condition, 'position DESC', 'id' );
        //подсчет курсов
