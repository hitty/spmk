...
// добавление title
$this_page->manageMetadata( array( 'title' => 'металлоконструкции' ) );
...
switch( $action ){
        ////////////////////////////////////////////////////////////////////////////////////////////////////////
        // фотогалерея металлоконструкций
        ////////////////////////////////////////////////////////////////////////////////////////////////////////
        case 'photos':
            if($ajax_mode){
                ...
                switch($action){
                    case 'list':
                        ...
                        if(!empty($id)){
                            $list = Photos::getList( 'metallokonstrukcii', $id );
                            if(!empty($list)){
                                $ajax_result['ok'] = true;
                    ...
                    case 'add':
                        ...
                        $id = Request::GetInteger('id', METHOD_POST);  
                        if(!empty($id)){
                            $res = Photos::Add( 'metallokonstrukcii', $id );
                            if(!empty($res)){
                    ...
                    case 'del':
                        ...
                        if( !empty( $id ) ) {
                            $res = Photos::Delete( 'metallokonstrukcii', $id );
                            if( !empty( $res ) ) {
                                $ajax_result['ok'] = true;
                    ...
                    case 'setMain':
                        ...
                        if(!empty($id_photo)){
                            $res = Photos::setMain( 'metallokonstrukcii', $id, $id_photo);
                            if(!empty($res)){
                                $ajax_result['ok'] = true;
                    ...
                    case 'rotate':
                        ...
                        $id_photo = Request::GetInteger( 'id_photo', METHOD_POST );                
                        if(!empty($id_photo)){
                            $res = Photos::rotatePhoto( 'metallokonstrukcii', $id_photo );
                            if(!empty($res)){
                                $ajax_result['ok'] = true;
                    ...
                    case 'setTitle':
                        ...
                        if(!empty($id)){
                            $res = Photos::setTitle('metallokonstrukcii',$id, $title);
    ...
    /****************************\
    |* Редактирование металлоконструкций  *|
    \***************************/        
    case 'add':
    case 'edit':
        ...
        if($action=='add'){
            ...
            $info = $db->prepareNewRecord($sys_tables['metallokonstrukcii']);
            //временный ID для связанных таблиц
            if( empty( Session::GetInteger( 'common_metallokonstrukcii_add' ) ) ) {
                $common_metallokonstrukcii_add = mt_rand( 800000000, 1000000000 );
                Session::SetInteger( 'common_metallokonstrukcii_add', $common_metallokonstrukcii_add );
            }
            Response::SetInteger( 'common_edit', Session::GetInteger( 'common_metallokonstrukcii_add' ) );
            
        } else {
            // получение данных из БД
            $info = $db->fetch("SELECT *
                                FROM ".$sys_tables['metallokonstrukcii']." 
                                WHERE id=?", $id) ;
            if( empty( $info ) ) Host::Redirect( '/manage/metallokonstrukcii/add/' );
            Response::SetInteger( 'id_metallokonstrukcii', $id );
        }
        // перенос дефолтных (считанных из базы) значений в мэппинг формы
        foreach( $info as $key=>$field ){
            if( !empty( $mapping['metallokonstrukcii'][$key] ) ) $mapping['metallokonstrukcii'][$key]['value'] = $info[$key];
        }
        ...
        if( !empty( $post_parameters['submit'] ) ){
            ...
            foreach( $post_parameters as $key => $field ){
                if( !empty( $mapping['metallokonstrukcii'][$key] ) ) $mapping['metallokonstrukcii'][$key]['value'] = $post_parameters[$key];
            }
            // проверка значений из формы
            $errors = Validate::validateParams( $post_parameters, $mapping['metallokonstrukcii'] );
            // выписывание ошибок в мэппинг формы (для отображения ошибочных полей)
            foreach( $errors as $key => $value ){
                if( !empty( $mapping['metallokonstrukcii'][$key] ) ) $mapping['metallokonstrukcii'][$key]['error'] = $value;
            }
            ...
            if( empty( $errors ) ) {
                ...
                foreach($info as $key=>$field){
                    if(isset($mapping['metallokonstrukcii'][$key]['value'])) $info[$key] = $mapping['metallokonstrukcii'][$key]['value'];
                }
                ...
                // сохранение в БД
                if($action=='edit'){
                    $res = $db->updateFromArray($sys_tables['metallokonstrukcii'], $info, 'id') or die($db->error);
                } else {
                    $res = $db->insertFromArray($sys_tables['metallokonstrukcii'], $info, 'id');
                    if(!empty($res)){
                        $new_id = $db->insert_id;
                        
                        //обновление временных данных
                        //перенос фотографий
                        Photos::setMain( 'metallokonstrukcii', $new_id, false, 'id_parent',  Session::GetInteger( 'common_metallokonstrukcii_add' ) );
                        Session::SetInteger( 'common_metallokonstrukcii_add', 0 );
                        // редирект на редактирование свеженькой страницы
                        if(!empty($res) && empty( $ajax_mode ) ) {
                            header('Location: '.Host::getWebPath('/manage/metallokonstrukcii/edit/'.$new_id.'/'));
                            exit(0);
                        }
                    }
                }
            ...
            if( $action == 'add' ) {
                ...
                Photos::DeleteAll( 'metallokonstrukcii', false, Session::GetInteger( 'common_metallokonstrukcii_add' ) );
            }
        }
        ...
        Response::SetArray( 'data_mapping', $mapping['metallokonstrukcii'] );

        if( ! ( !empty( $ajax_mode ) && !empty( $post_parameters['submit'] ) ) )  $module_template = '/modules/metallokonstrukcii/manage/templates/edit.html';
    ...
    case 'delete':
        if( $ajax_mode ){
            ...
            if( empty( $post_parameters['submit'] ) ) {
                ...
                Response::SetString( 'title', 'металлоконструкции' );
                $ajax_result = array('ok' => true );
            } else {
                //удаление
                $id = empty($this_page->page_parameters[2]) ? 0 : $this_page->page_parameters[2];
                
                $res = $db->query("DELETE FROM ".$sys_tables['metallokonstrukcii']." WHERE id=?", $id);
        ...
        break;
    default:
        $module_template = '/modules/metallokonstrukcii/manage/templates/list.html';
        // формирование списка
        $conditions = array( );
        if(!empty($filters)){
            if(!empty($filters['title'])) $conditions['title'] = $sys_tables['metallokonstrukcii'].".`title` LIKE '%".$db->real_escape_string($filters['title'])."%'";
            if(!empty($filters['published'])) $conditions['published'] = $sys_tables['metallokonstrukcii'].".`published` = ".$db->real_escape_string($filters['published'])."";
        }
        // формирование списка для фильтра
        $condition = implode(" AND ",$conditions);        
        // создаем пагинатор для списка
        $paginator = new Paginator( $sys_tables['metallokonstrukcii'], 30, $condition );
        ...
        // ссылка пагинатора
        $paginator->link_prefix = '/manage/metallokonstrukcii'                // модуль
        ...
        // формирование списка
        $list = CommonDb::getList( 'metallokonstrukcii', $paginator->getLimitString($page), $condition, 'position DESC', 'id' );
        //подсчет курсов
