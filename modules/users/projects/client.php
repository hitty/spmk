...
$GLOBALS['css_set'][] = '/modules/users/projects/css/style.css';
$GLOBALS['js_set'][] = '/modules/users/projects/js/script.js';
...
switch($action){
    ...
    case 'edit':
        ...
        $module_template = '/modules/users/projects/templates/edit.html';
        ...
        if($action=='add'){
            ...
            $info = $db->fetch("SELECT 
                                ...
                                WHERE id=?", $id) ;        
            if( empty( $info ) ) Host::Redirect( '/projects/users/add/' );
            ...
            if(empty($errors)) {
                ...
                if($action=='edit'){
                    ...
                    if( !empty( $res ) ) {
                        ...
                        if(!empty($res)) {
                            header('Location: '.Host::getWebPath('/projects/users/edit/'.$new_id.'/'));
                            exit(0);
                        }
                    }
                }
        ...
        break;
    default:
        $module_template = '/modules/users/projects/templates/list.html';
        // формирование списка
        $conditions = array( );
        if( !empty( $filters ) ){
