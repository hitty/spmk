...
$GLOBALS['css_set'][] = '/modules/uslugi/manage/css/style.css';
$GLOBALS['js_set'][] = '/modules/uslugi/manage/js/script.js';
// мэппинги модуля
$mapping = include( dirname( __FILE__ ) . '/mapping.php' );

// добавление title
$this_page->manageMetadata( array( 'title' => 'Услуги' ) );
...
switch( $action ){
            ...
            if($ajax_mode){
                ...
                switch($action){
                    ...
                    case 'setMain':
                        ...
                        if(!empty($id_photo)){
                            $res = Photos::setMain( 'uslugi', $id, $id_photo);
                            if(!empty($res)){
                                $ajax_result['ok'] = true;
                    ...
                    case 'rotate':
                        ...
                        $id_photo = Request::GetInteger( 'id_photo', METHOD_POST );                
                        if(!empty($id_photo)){
                            $res = Photos::rotatePhoto( 'uslugi', $id_photo );
                            if(!empty($res)){
                                $ajax_result['ok'] = true;
                    ...
                    case 'setTitle':
                        ...
                        if(!empty($id)){
                            $res = Photos::setTitle('uslugi',$id, $title);
    ...
    case 'edit':
        ...
        if($action=='add'){
            ...
            $info = $db->prepareNewRecord($sys_tables['uslugi']);
            //временный ID для связанных таблиц
            if( empty( Session::GetInteger( 'common_uslugi_add' ) ) ) {
                $common_uslugi_add = mt_rand( 800000000, 1000000000 );
                Session::SetInteger( 'common_uslugi_add', $common_uslugi_add );
            }
            Response::SetInteger( 'common_edit', Session::GetInteger( 'common_uslugi_add' ) );
            
        } else {
            // получение данных из БД
            $info = $db->fetch("SELECT *
                                FROM ".$sys_tables['uslugi']." 
                                WHERE id=?", $id) ;
            if( empty( $info ) ) Host::Redirect( '/manage/uslugi/add/' );
            Response::SetInteger( 'id_uslugi', $id );
        }
        // перенос дефолтных (считанных из базы) значений в мэппинг формы
        foreach( $info as $key=>$field ){
            if( !empty( $mapping['uslugi'][$key] ) ) $mapping['uslugi'][$key]['value'] = $info[$key];
        }
        ...
        if( !empty( $post_parameters['submit'] ) ){
            ...
            foreach( $post_parameters as $key => $field ){
                if( !empty( $mapping['uslugi'][$key] ) ) $mapping['uslugi'][$key]['value'] = $post_parameters[$key];
            }
            // проверка значений из формы
            $errors = Validate::validateParams( $post_parameters, $mapping['uslugi'] );
            // выписывание ошибок в мэппинг формы (для отображения ошибочных полей)
            foreach( $errors as $key => $value ){
                if( !empty( $mapping['uslugi'][$key] ) ) $mapping['uslugi'][$key]['error'] = $value;
            }
            ...
            if( empty( $errors ) ) {
                ...
                foreach($info as $key=>$field){
                    if(isset($mapping['uslugi'][$key]['value'])) $info[$key] = $mapping['uslugi'][$key]['value'];
                }
                // сохранение в БД
                if($action=='edit'){
                    $info['chpu_title'] = Convert::chpuTitle( $info['title'] );
                    $res = $db->updateFromArray($sys_tables['uslugi'], $info, 'id') or die($db->error);
                } else {
                    $res = $db->insertFromArray($sys_tables['uslugi'], $info, 'id');
                    if(!empty($res)){
                        ...
                        //перенос фотографий
                        Photos::setMain( 'uslugi', $new_id, false, 'id_parent',  Session::GetInteger( 'common_uslugi_add' ) );
                        Session::SetInteger( 'common_uslugi_add', 0 );
                        // редирект на редактирование свеженькой страницы
                        if(!empty($res) && empty( $ajax_mode ) ) {
                            header('Location: '.Host::getWebPath('/manage/uslugi/edit/'.$new_id.'/'));
                            exit(0);
                        }
                    }
                }
            ...
            if( $action == 'add' ) {
                ...
                Photos::DeleteAll( 'uslugi', false, Session::GetInteger( 'common_uslugi_add' ) );
            }
        }
        ...
        Response::SetArray( 'data_mapping', $mapping['uslugi'] );

        if( ! ( !empty( $ajax_mode ) && !empty( $post_parameters['submit'] ) ) )  $module_template = '/modules/uslugi/manage/templates/edit.html';
    ...
    case 'delete':
        if( $ajax_mode ){
            ...
            if( empty( $post_parameters['submit'] ) ) {
                ...
                Response::SetString( 'title', 'Услуги' );
                $ajax_result = array('ok' => true );
            } else {
                //удаление
                $id = empty($this_page->page_parameters[1]) ? 0 : $this_page->page_parameters[1];
                
                $res = $db->query("DELETE FROM ".$sys_tables['uslugi']." WHERE id=?", $id);
        ...
        break;
    default:
        $module_template = '/modules/uslugi/manage/templates/list.html';
        // формирование списка
        $conditions = array( );
        if(!empty($filters)){
            if(!empty($filters['title'])) $conditions['title'] = $sys_tables['uslugi'].".`title` LIKE '%".$db->real_escape_string($filters['title'])."%'";
            if(!empty($filters['published'])) $conditions['published'] = $sys_tables['uslugi'].".`published` = ".$db->real_escape_string($filters['published'])."";
        }
        // формирование списка для фильтра
        $condition = implode(" AND ",$conditions);        
        // создаем пагинатор для списка
        $paginator = new Paginator( $sys_tables['uslugi'], 30, $condition );
        ...
        // ссылка пагинатора
        $paginator->link_prefix = '/manage/uslugi'                // модуль
        ...
        // формирование списка
        $list = CommonDb::getList( 'uslugi', $paginator->getLimitString($page), $condition, 'position DESC', 'id' );
        //подсчет курсов
