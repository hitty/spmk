...
$GLOBALS['js_set'][] = '/modules/seo/projects/js/script.js';
$GLOBALS['css_set'][] = '/modules/seo/projects/css/style.css';
...
switch($action){
    ...
    case 'edit':
        ...
        $module_template = '/modules/seo/projects/templates/edit.html';
		...
		if($action=='add'){
            ...
            $info = $db->fetch("SELECT *
                                ...
                                WHERE id=?", $id);
            if(empty($info)) Host::Redirect('/projects/seo/add/');
        }
            ...
            if(empty($errors)) {
                ...
                if($action=='edit'){
                    ...
                    if(!empty($res)){
                        ...
                        if(!empty($res)) {
                            header('Location: '.Host::getWebPath('/projects/seo/edit/'.$new_id.'/'));
                            exit(0);
                        }
                    }
                }
        ...
        break;
    default:
        $module_template = '/modules/seo/projects/templates/list.html';
        // формирование списка
        $conditions = array();
        if(!empty($filters)){
        ...
        // ссылка пагинатора
        $paginator->link_prefix = '/projects/seo'                                  // модуль
