...
function make_pages_tree($parent_id=0, $nodes_expanded='all', $exclude_id = null){
    ...
    $nodes = $db->fetchall(
        "SELECT a.id,a.title,a.alias,a.url,
         ...
         LEFT JOIN ".$sys_tables['common_pages_map']." b ON a.id=b.object_id
         LEFT JOIN (SELECT parent_id, count(*) as quantity FROM ".$sys_tables['common_pages_map']." GROUP BY parent_id) c ON b.id=c.parent_id
         WHERE b.parent_id=?
...
function delete_page($id){
    ...
    $page_in_map = $db->fetch("
        SELECT id, path 
        FROM ".$sys_tables['common_pages_map']."
        WHERE object_id=?"
        , $id
    );
    if(empty($page_in_map)) return false;
    $del_ids = $db->fetchall("SELECT object_id FROM ".$sys_tables['common_pages_map']." WHERE path LIKE CONCAT(?,'.%')", 'object_id', $page_in_map['path']);
    if(empty($del_ids)) $del_ids = array();
    $del_ids[$id] = array('object_id' => $id);
    $del_ids = array_keys($del_ids);
    // удаление из pages
    $d2 = $db->query("DELETE FROM ".$sys_tables['common_pages']." WHERE id IN (".implode(',',$del_ids).")");
    // удаление из common_pages_map
    $d1 = $db->query("DELETE FROM ".$sys_tables['common_pages_map']." WHERE path LIKE CONCAT(?,'.%') OR id=?",$page_in_map['path'],$page_in_map['id']);
...
function move_page($page_id, $target_map_id){
    ...
    $object = $db->fetch("SELECT * FROM ".$sys_tables['common_pages_map']." WHERE object_id=?",$page_id);
    ...
    if($target_map_id<1){
        ...
        $target = $db->fetch("SELECT * FROM ".$sys_tables['common_pages_map']." WHERE id=?",$target_map_id);
    ...
    $result = $db->query("UPDATE ".$sys_tables['common_pages_map']."
...
function add_page_in_map($page_id, $map_parent_id){
    ...
    if($map_parent_id<1){
        ...
        $target = $db->fetch("SELECT id,level,path FROM ".$sys_tables['common_pages_map']."
                              WHERE id=?", $map_parent_id);
    }
    if(empty($target)) return false; // некуда добавлять
    $order_select = $db->fetch("SELECT max(position) as max_position FROM ".$sys_tables['common_pages_map']."
                                WHERE parent_id=?", $map_parent_id);
    if(empty($order_select) || $order_select['max_position']===null) $order_select = array('max_position'=>0);
    
    $result = $db->query("INSERT INTO ".$sys_tables['common_pages_map']."
    ...
    if(!$result) return false;
    $id = $db->insert_id;
    $result = $db->query("UPDATE ".$sys_tables['common_pages_map']." SET path=? WHERE id=?"
...
function get_parent_url($parent_id){
    global $db;
    $sys_tables = Config::$sys_tables;
    $parent = $db->fetch("SELECT a.url FROM ".$sys_tables['common_pages']." a
                          LEFT JOIN ".$sys_tables['common_pages_map']." b ON a.id=b.object_id
