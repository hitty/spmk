<?php
$GLOBALS['css_set'][] = '/modules/pages/manage/css/style.css';
$GLOBALS['js_set'][] = '/modules/pages/manage/js/script.js';
// добавление title
$this_page->manageMetadata(array('title'=>'Страницы'));
// основной шаблон модуля (шаблон по умолчанию)
$module_template = '/modules/pages/manage/templates/list.html';
// мэппинги модуля
...
if( !empty(  $this_page->page_parameters[0]  ) ){
    ...
    switch( $this_page->page_parameters[0] ){
        case 'add':
        case 'edit':
            $module_template = '/modules/pages/manage/templates/edit.html';
            if( $this_page->page_parameters[0] =='add'){
                ...
                $info = $db->fetch("SELECT a.*,b.id as map_id, b.parent_id as map_parent_id, b.path as map_path
                                    ...
                                    WHERE a.id=?", $id);
                if(empty($info)) Host::Redirect('/manage/pages/add/');
            }
                ...
                if(empty($errors)) {
                    ...
                    if( $this_page->page_parameters[0] =='edit'){
                        ...
                        if(!empty($res)){
                            ...
                            if(!empty($res)) {
                                header('Location: '.Host::getWebPath('/manage/pages/edit/'.$new_id.'/'));
                                exit(0);
                            }
                        }
                    }
