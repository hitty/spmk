...
class Host {
    ...
    public static $domain = "";                 // домен
    public static $subdomain = "";              // поддомен
    ...
    private static $local_ip_list = array("92.255.25.34","46.72.119.83","92.255.25.36");
    ...
    */
    public static function Init(){
        ...
        if(strpos(self::$requested_uri, '//') != '' && strpos(self::$requested_uri, '/?') == '') self::Redirect(rtrim(self::$requested_uri,'/'));
        ...
        $url_parts = explode ( ".", self::$host );
        self::$subdomain = ltrim( count( $url_parts ) == 3 ? $url_parts[0] : '', 'www.' );
        self::$domain = ltrim( self::$host, self::$subdomain . '.' );
    ...
    */
    public static function Redirect( $location, $type = 301 ) {
        $location = self::GetWebPath( $location );
        if($type == 301){
            Session::SetBoolean('redirect_301', true);
            header( "Location: " . $location .(strstr($location,'?')=='' && strlen($location)>19 ? '/' : ''), true, 301);
        } else {
    ...
    */
    public static function RedirectLevelUp(  ) {
        ...
        self::Redirect( trim( '/' . implode('/', $location) . '/', '//' ) . '/' );
        exit();
    }    
    /**
    ...
    * @return boolean
    */    
    public static function isLocalHost($tablename = false, $id_to_click = false) {
        global $db;
        $ip = self::getUserIp();
        if(!in_array($ip,self::$local_ip_list)) return false;
