...
class Paginator {
    ...
    * @return Paginator
    */
    public function __construct($tablename, $limit=null, $where = '', $sql_full = '', $group_by = false, $uri = false){
        ...
        //страница
        $this->current_page = Request::GetInteger( 'page', METHOD_GET );
        //редирект с несуществующих пейджей
        if( empty( $this->current_page ) )  $this->current_page = 1;
        //построение списка ссылок
        if( !empty( $uri ) ) $this->Links( $uri );
        if( $this->pages_count > 0 && $this->pages_count < $this->current_page ) Host::RedirectLevelUp();
    }
    ...
    */
    public function Get($current_page=1){
        ...
        $result =  array(
            ...
            'sort_link' => $this->sort_link_prefix,
    ...
    public function Links( $uri ){
            
        $parsed_url = parse_url( $uri );
        $uri = '/' . trim( $parsed_url['path'], '/' );
        ...
        // get-параметры для ссылок сортировки
        $get_in_sortinator = array();
        foreach( $get_parameters as $gk=>$gv) if( $gk!='sort') $get_in_sortinator[] = $gk.'='.$gv;
        // ссылка пагинатора
        $this->sort_link_prefix = rtrim( $uri, '/' ) . "/?"                              // конечный слеш и начало GET-строки
                                  .implode( '&', $get_in_sortinator )                // GET-строка
                                  .(empty( $get_in_sortinator ) ? "" : '&' )."sort=";// параметр для номера страницы

        if( $this->pages_count > 0 && $this->pages_count < $this->current_page ){
            Header('Location: '.Host::getWebPath( $this->link_prefix . $this->pages_count ) ) ;
            exit(0);
        }
        Response::SetArray( 'paginator', $this->Get( $this->current_page ) ) ;
