...
if(!defined("TYPE_URL")) define( "TYPE_URL", "url" );
...
class Convert {
    ...
    */
    public static function StripText($string){
        return preg_replace('/<([a-z][a-z0-9]*)[^>]*?(\/?)>/i', '<$1$2>', strip_tags($string,'<div><p><ul><ol><li><b><strong><i><h3><h4>'));
    }

    /**
    * Замена в url человекоподобный формат
    ...
    * @return  string
    */     
    public static function filesizeInBytes( $bytes, $decimals = 2 ) {
    ...
    */    
    ...
    public static function stripUnwantedTagsAndAttrs($html_str){
        if(!empty($html_str)){
            ...
            foreach($html->find('strong,b,i,em,ol,ul,li,p,div,span,br,h1,h2,h3,blockquote,font') as $p) {
...
class Validate{
    ...
    public static function isValidType( $value, $type ) {
        switch( $type ) {
            ...
            case TYPE_PARAMETER:
                return true;
            case TYPE_URL:
                return !empty( get_headers( $value, 1) );
            default:
                return false;
        }
    }
    ...
    */
    public static function isEmail( $email ) {
        return (bool) preg_match( '!^([a-z0-9_-]+\.)*[a-z0-9_-]+@[a-z0-9_-]+(\.[a-z0-9_-]+)*\.[a-z]{2,6}$!i', (string) $email);
    } 
                    ...
                    switch($type){
                        case 'type':
                            switch($critery){
                                ...
                                case 'url' :
                                    $mapping[$key]['value'] = Convert::ToString( $value );
                                    $critery_text = 'существующим url\'ом';
                                    break;
                            }
