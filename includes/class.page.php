...
class Page {
    ...
    */
    ...
    public function Render( $is_block = false){
        ...
        $static_pages = $db->fetch( " SELECT * FROM " . $sys_tables['static_pages'] . " WHERE url LIKE ?", $temp_path );
        if( !empty( $static_pages ) ) $temp_path = 'static_pages';
        ...
        if( empty( $page) || (strlen( $page['url'])<strlen( $this->real_path) && $page['no_require_params']==1 ) ) {
            ...
            if( !$this->checkAccess( ) ){
                ...
                //настройки сайта
                $settings = CommonDb::getList( 'settings' );
                foreach( $settings as $k => $value ) $this_page->settings[ $value['title'] ] = $value['value'];
                Response::SetArray( 'settings', $this_page->settings );
                
                $this_page->menuClear( 1 );
                $this_page->menuAdd( 'О премии', 'about', 1 );
                $this_page->menuAdd( 'Новости', 'news', 1 );
                $this_page->menuAdd( 'Фото и видео', 'media', 1 );
                $this_page->menuAdd( 'Номинанты', 'nominees', 1 );
                $this_page->menuAdd( 'Жюри', 'jury', 1 );
                //if( !empty( $auth->id ) && ( $auth->admin == 1 || ( $auth->admin == 2 && !empty( $this_page->settings['applications_voting'] )  && $this_page->settings['applications_voting'] == 1 ) ) ) $this_page->menuAdd( 'Голосование', 'voting', 1, false, false, false );
                ...
                if( $this->http_code==200){
                    if( $this->first_instance){
                        ...
                        //года для архива
                        $archive_years = CommonDb::getList( 'nominees_years', false, 'actual = 2 AND title <= YEAR(CURDATE())', 'title DESC' );
                        Response::SetArray( 'archive_years', $archive_years );
                    }
