...
class Page
{
    ...
    public function Render($is_block = false)
    {
        ...
        //Продакция
        $productions = $db->fetchall(" SELECT * FROM " . $sys_tables['assortment'] . " WHERE published = 1");
        Response::SetArray('productions', $productions);
        //Объекты
        $objects = $db->fetchall(" SELECT * FROM " . $sys_tables['objects'] . " WHERE published = 1");
        ...
        if (empty($page) || (strlen($page['url']) < strlen($this->real_path) && $page['no_require_params'] == 1)) {
            ...
            if (!$this->checkAccess()) {
                ...
                if ($this->http_code == 200) {
                    ...
                    if (empty($h1) && !empty($page_seo['h1_title'])) Response::SetString('h1', $page_seo['h1_title']);
    ...
    public function menuAdd($title, $url, $level = 1, $active = false, $class = false, $child = false, $reset_active = false)
    {
        ...
        if (!empty($child)) {
            ...
            array_push($this->menu[$menulevel][$child], array('title' => $title, 'url' => $url, 'active' => $active_state, 'class' => $class, 'child' => $child, 'amount' => false, 'external_link' => !empty($external_link) ? true : false));
